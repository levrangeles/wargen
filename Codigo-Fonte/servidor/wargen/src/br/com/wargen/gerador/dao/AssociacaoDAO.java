...
public class AssociacaoDAO {
	...
	public static void excluirAssociacaoPorId(int id, Connection conn) throws Exception {
		try {
			PreparedStatement stmt = conn.prepareStatement(DBManager.getInstance().getQueryFile().getProperty("excluirAssociacao"));
			stmt.setInt(1, id);		
			stmt.execute();
		}
		catch (Exception exc){
	...
	public static void inserirAssociacao(AssociacaoBean associacao, Connection conn) throws Exception {
		try {
			PreparedStatement stmt = conn.prepareStatement(DBManager.getInstance().getQueryFile().getProperty("inserirAssociacao"));
	...
	public static AssociacaoBean carregarAssociacaoPorId(int id, Connection conn) throws Exception {
		try {
			PreparedStatement stmt = conn.prepareStatement(DBManager.getInstance().getQueryFile().getProperty("carregarAssociacaoPorId"));
			stmt.setInt(1, id);
	...
	public static ArrayList<AssociacaoBean> listarAssociacoes(Connection conn) throws Exception {
		...
		try {
			PreparedStatement stmt = conn.prepareStatement(new DBManager().getQueryFile().getProperty("listarAssociacoes"));
